...
const CATEGORY_COLOR: Record<EssayHintCategory, { bg: string; text: string }> = {
  ...
  vocabulary: { bg: 'var(--color-soft-amber)', text: 'var(--color-orange-title)' },
    ...
    <div className="flex flex-col gap-2">
      {hints.map((h) => {
        ...
        return (
          <div
            key={h.id}
            className="flex items-start gap-3 rounded-2xl border border-(--color-surface) bg-(--color-surface)/95 px-4 py-3 shadow-[0_4px_10px_rgba(0,0,0,0.045)] md:px-5"
          >
            <span
