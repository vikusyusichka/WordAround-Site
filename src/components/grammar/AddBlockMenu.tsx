  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex h-11 items-center justify-center gap-2 rounded-2xl border border-dashed border-(--color-auth-field-border) bg-(--color-surface)/60 text-[14px] font-semibold text-(--color-primary-blue) transition-colors hover:bg-(--color-surface) focus-visible:outline-none"
      >
      ...
      {open && (
        <div className="grid grid-cols-2 gap-2 rounded-2xl border border-(--color-surface) bg-(--color-surface)/95 p-2 shadow-[0_4px_10px_rgba(0,0,0,0.045)] sm:grid-cols-4">
          {types.map((type) => (
            <button
              key={type}
              type="button"
