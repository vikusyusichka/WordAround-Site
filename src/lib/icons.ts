/* Mapping from SF Symbols (used throughout SwiftUI code) to Phosphor icons.
   Phosphor was picked because its `bold` weight matches SF Symbols'
   default semibold rendering, and its `fill` weight is the direct
   equivalent of iOS `.fill` symbol variants — the visual DNA is much
   closer than what Lucide gives us. Populated as we port screens;
   unknown names fall back to <Question /> so gaps fail visibly. */
import {
  Airplane,
  ArrowDown,
  ArrowLeft,
  ArrowRight,
  ArrowsLeftRight,
  ArrowUp,
  Bell,
  Book,
  BookmarkSimple,
  BookOpen,
  Books,
  Brain,
  Briefcase,
  CaretDown,
  CaretRight,
  ChartBar,
  ChatCircleDots,
  ChatsCircle,
  Check,
  Circle,
  Confetti,
  DotsSixVertical,

  FileText,
  Flame,
  Folder,
  FolderPlus,
  ForkKnife,
  Gear,
  Globe,
  GraduationCap,
  Headphones,
  Heart,
  ArrowClockwise,
  ChatTeardropText,
  Cube,
  Hourglass,
  House,
  Image as ImageIcon,
  Palette,
  CheckCircle,
  Clock,
  FilePlus,
  FilmStrip,
  Flag,
  Lightning,
  Lightbulb,
  ListBullets,
  ListChecks,
  MagicWand,
  Megaphone,
  Microphone,
  Minus,
  NotePencil,
  Play,
  Pencil,
  PencilLine,
  PersonSimpleWalk,
  Plus,
  Question,
  Quotes,
  Rabbit,
  SealCheck,
  Shuffle,
  Sparkle,
  SquaresFour,
  Stack,
  Star,
  Target,
  TextAa,
  TextAlignLeft,
  TextHOne,
  ThumbsDown,
  ThumbsUp,
  Translate,
  Trash,
  Tray,
  UsersThree,
  WarningCircle,
  XCircle,
  User,
  UserCircle,
  Warning,
  Waveform,
  X,
  AppleLogo,
  Bandaids,
  Bed,
  Bicycle,
  Broadcast,
  Bus,
  Cake,
  Calendar,
  CalendarCheck,
  Camera,
  Car,
  Carrot,
  ChartPie,
  ChatCenteredText,
  Clipboard,
  ClipboardText,
  Cloud,
  CloudRain,
  Coffee,
  Cpu,
  Desktop,
  DeviceMobile,
  DeviceTablet,
  Drop,
  FirstAidKit,
  GlobeHemisphereWest,
  Guitar,
  Hamburger,
  HardDrives,
  Keyboard,
  Laptop,
  Leaf,
  Link,
  ListNumbers,
  MagnifyingGlass,
  MapTrifold,
  Moon,
  MusicNote,
  NavigationArrow,
  Note,
  PaintBrush,
  PaperPlaneTilt,
  PushPin,
  PersonSimpleRun,
  PersonSimpleTaiChi,
  Pill,
  Scissors,
  Snowflake,
  SortAscending,
  SpeakerHigh,
  Stethoscope,
  Suitcase,
  Sun,
  Tag,
  Train,
  Tree,
  Trophy,
  VideoCamera,
  WifiHigh,
  Wine,
  type Icon as PhosphorIcon,
} from '@phosphor-icons/react';

/* SF-Symbol name (as written in Swift source) → Phosphor component.
   Both outlined and .fill variants map to the same component — the Icon
   wrapper reads the trailing `.fill` off the name to pick the weight. */
const SF_TO_PHOSPHOR: Record<string, PhosphorIcon> = {
  house: House,
  'house.fill': House,
  folder: Folder,
  'folder.fill': Folder,
  plus: Plus,
  'square.stack.3d.up': Stack,
  'square.stack.3d.up.fill': Stack,
  person: User,
  'person.fill': User,
  'person.crop.circle.fill': UserCircle,
  'bubble.left.and.bubble.right': ChatsCircle,
  headphones: Headphones,
  book: BookOpen,
  'book.closed': BookOpen,
  'pencil.and.scribble': PencilLine,
  'chart.bar.fill': ChartBar,
  target: Target,
  'flame.fill': Flame,
  bell: Bell,
  circle: Circle,
  'arrow.left': ArrowLeft,
  'arrow.right': ArrowRight,
  'chevron.right': CaretRight,
  'chevron.down': CaretDown,
  xmark: X,
  sparkles: Sparkle,
  sparkle: Sparkle,
  checkmark: Check,
  pencil: Pencil,
  waveform: Waveform,
  mic: Microphone,
  'mic.fill': Microphone,
  photo: ImageIcon,
  'doc.text.fill': FileText,
  /* Stub flashcard-set icons (Phase 2 demo data; real icons come from
     set.icon in Phase 3). */
  airplane: Airplane,
  'fork.knife': ForkKnife,
  'briefcase.fill': Briefcase,
  /* Create-set preview icons + suggester outputs. */
  'rectangle.stack.fill': Stack,
  'book.closed.fill': BookOpen,
  'book.fill': BookOpen,
  'text.book.closed.fill': BookOpen,
  'graduationcap.fill': GraduationCap,
  'brain.head.profile': Brain,
  'globe.europe.africa.fill': Globe,
  'star.fill': Star,
  'heart.fill': Heart,
  'bolt.fill': Lightning,
  'pencil.and.outline': Pencil,
  'character.bubble.fill': Translate,
  /* Phase 4 Writing */
  'square.grid.2x2.fill': SquaresFour,
  'note.text.badge.plus': NotePencil,
  'book.pages.fill': Book,
  lightbulb: Lightbulb,
  'lightbulb.fill': Lightbulb,
  'arrow.right.circle.fill': ArrowRight,
  'arrow.uturn.backward': ArrowLeft,
  /* Phase 4C3 helper toolbar */
  'character.book.closed.fill': Translate,
  'textformat.abc.dottedunderline': TextAa,
  /* Phase 4B WriteWords difficulty + result */
  'hare.fill': Rabbit,
  'figure.walk': PersonSimpleWalk,
  'party.popper.fill': Confetti,
  hourglass: Hourglass,
  gearshape: Gear,
  'gearshape.fill': Gear,
  /* Phase 4D GrammarNotes — note types + block types + editor */
  'arrow.left.arrow.right': ArrowsLeftRight,
  'checklist.checked': ListChecks,
  checklist: ListChecks,
  'text.alignleft': TextAlignLeft,
  'text.word.spacing': TextAlignLeft,
  'textformat.size.larger': TextHOne,
  'textformat.size': TextHOne,
  'list.bullet': ListBullets,
  'quote.opening': Quotes,
  minus: Minus,
  'arrow.up': ArrowUp,
  'arrow.down': ArrowDown,
  trash: Trash,
  'exclamationmark.triangle.fill': Warning,
  /* Phase 4D2 grammar quiz */
  'pencil.line': PencilLine,
  'square.and.pencil': NotePencil,
  'wand.and.stars': MagicWand,
  'checkmark.circle': CheckCircle,
  'checkmark.circle.fill': CheckCircle,
  'questionmark.circle.fill': Question,
  'play.fill': Play,
  /* Phase 4D3 spaced review */
  'xmark.circle.fill': XCircle,
  'checkmark.seal.fill': SealCheck,
  'tray.fill': Tray,
  /* Phase 4D4 templates */
  'clock.fill': Clock,
  'flag.fill': Flag,
  'doc.badge.plus': FilePlus,
  /* Phase 4D5 quick mistake */
  'exclamationmark.bubble.fill': ChatCircleDots,
  'exclamationmark.circle.fill': WarningCircle,
  /* Phase 5 Reading */
  'books.vertical.fill': Books,
  /* Phase 6 Listening */
  'waveform.badge.plus': Waveform,
  'film.stack': FilmStrip,
  'bookmark.fill': BookmarkSimple,
  /* Phase 7 Speaking */
  'bubble.left.and.bubble.right.fill': ChatsCircle,
  'person.2.fill': UsersThree,
  'cube.box.fill': Cube,
  'hand.thumbsup.fill': ThumbsUp,
  'hand.thumbsdown.fill': ThumbsDown,
  shuffle: Shuffle,
  'paintpalette.fill': Palette,
  'text.bubble.fill': ChatTeardropText,
  'photo.fill': ImageIcon,
  'arrow.clockwise': ArrowClockwise,
  'megaphone.fill': Megaphone,
  'list.bullet.rectangle.fill': ListChecks,
  /* Set-icon catalog (mirrors iOS SFSymbolCatalog) — Learning */
  'note.text': Note,
  'note.text.fill': Note,
  /* Notes: pin / favorite / search / reorder / block types (4E). */
  pin: PushPin,
  'pin.fill': PushPin,
  star: Star,
  magnifyingglass: MagnifyingGlass,
  'list.number': ListNumbers,
  'pencil.and.list.clipboard': ClipboardText,
  'arrow.up.arrow.down': SortAscending,
  'folder.badge.plus': FolderPlus,
  'line.3.horizontal': DotsSixVertical,
  'clipboard.fill': Clipboard,
  /* Languages */
  globe: Globe,
  'globe.americas.fill': GlobeHemisphereWest,
  'quote.bubble.fill': ChatCenteredText,
  translate: Translate,
  'speaker.wave.2.fill': SpeakerHigh,
  /* Nature */
  'leaf.fill': Leaf,
  'tree.fill': Tree,
  'sun.max.fill': Sun,
  'moon.fill': Moon,
  'cloud.fill': Cloud,
  'cloud.rain.fill': CloudRain,
  'drop.fill': Drop,
  snowflake: Snowflake,
  /* Health */
  'cross.case.fill': FirstAidKit,
  stethoscope: Stethoscope,
  'bandage.fill': Bandaids,
  'pills.fill': Pill,
  'bed.double.fill': Bed,
  'figure.run': PersonSimpleRun,
  'figure.cooldown': PersonSimpleTaiChi,
  /* Food */
  'cup.and.saucer.fill': Coffee,
  'wineglass.fill': Wine,
  'takeoutbag.and.cup.and.straw.fill': Hamburger,
  'birthday.cake.fill': Cake,
  'carrot.fill': Carrot,
  /* Travel */
  'car.fill': Car,
  'bus.fill': Bus,
  'tram.fill': Train,
  bicycle: Bicycle,
  'map.fill': MapTrifold,
  'location.fill': NavigationArrow,
  'suitcase.fill': Suitcase,
  /* Work */
  calendar: Calendar,
  'calendar.badge.clock': CalendarCheck,
  'chart.pie.fill': ChartPie,
  /* Tech — SF leans on Apple hardware here; Phosphor's generic devices are
     the closest honest match, and apple.logo has a real twin. */
  laptopcomputer: Laptop,
  desktopcomputer: Desktop,
  ipad: DeviceTablet,
  iphone: DeviceMobile,
  'apple.logo': AppleLogo,
  'keyboard.fill': Keyboard,
  'cpu.fill': Cpu,
  'server.rack': HardDrives,
  wifi: WifiHigh,
  'antenna.radiowaves.left.and.right': Broadcast,
  /* Creativity */
  'paintbrush.fill': PaintBrush,
  scissors: Scissors,
  'camera.fill': Camera,
  'video.fill': VideoCamera,
  'music.note': MusicNote,
  'guitars.fill': Guitar,
  /* General */
  'trophy.fill': Trophy,
  'bell.fill': Bell,
  'tag.fill': Tag,
  'paperplane.fill': PaperPlaneTilt,
  link: Link,
};

export const iconForSFSymbol = (name: string): PhosphorIcon =>
  SF_TO_PHOSPHOR[name] ?? Question;

export type SFSymbolWeight = 'thin' | 'light' | 'regular' | 'bold' | 'fill' | 'duotone';

/* SF Symbols with a `.fill` suffix render as solid on iOS. Everything else
   we render `bold`, which is the closest match to SwiftUI's default
   `.semibold` rendering under `.font(.system(size:weight:))`. */
export const weightForSFSymbol = (name: string): SFSymbolWeight =>
  name.endsWith('.fill') ? 'fill' : 'bold';
